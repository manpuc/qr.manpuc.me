...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (q) {
    try {
      ...
      if (decompressed) {
        ...
        // If transparent is set, we use white for OG image as social media previews don't always handle transparency well
        if (parsed.t === 1) {
            bgColor = "#ffffff";
        }
      }
  ...
  try {
    ...
    // We use a high quality scale and margin
